...
export function IncidentList({ incidents }: { incidents: AlertIncident[] }) {
  return (
    <Card>
      <h2 className="text-lg font-semibold text-ink">Active Incidents</h2>
      {incidents.length === 0 ? (
        ...
        <div className="mt-4 space-y-3">
          {incidents.map((incident) => (
            ...
            >
              ...
              <p className="mt-2 text-sm font-medium text-ink">{incident.message}</p>
              <p className="mt-1.5 font-mono text-xs text-muted">{formatDate(incident.createdAt)}</p>
            </article>
          ))}
        </div>
      )}
